...
# Автоматический подбор рабочей стратегии для Telegram
# ============================================================
...
RESULTS_FILE="$SCRIPT_DIR/auto_tune_telegram_results.txt"
...
declare -a WORKING_STRATEGIES=()  # Рабочие стратегии (номер:имя:web_ok:api_ok)
TESTED_COUNT=0
SUCCESS_COUNT=0
FAILED_COUNT=0
...
# Извлечь номер из записи (формат: номер:имя:web_ok:api_ok)
get_num_from_entry() {
    echo "${1%%:*}"
}
...
# ═══════════════════════════════════════════════════════════
# ФУНКЦИИ: Проверка Telegram
# ═══════════════════════════════════════════════════════════

# Проверка веб-интерфейса (HTTP код + размер + ключевое слово)
check_telegram_web() {
    local tmpfile=$(mktemp)
    local code=$(curl -s --tlsv1.3 --connect-timeout "$CURL_TIMEOUT" --max-time "$CURL_TIMEOUT" \
        -o "$tmpfile" -w "%{http_code}" "https://web.telegram.org" 2>/dev/null)
    local size=$(wc -c < "$tmpfile" 2>/dev/null || echo 0)
    local has_keyword=0
    grep -qi "telegram" "$tmpfile" 2>/dev/null && has_keyword=1
    rm -f "$tmpfile"
    
    [[ "$code" =~ ^[23] ]] && [[ $has_keyword -eq 1 ]]
}

# Проверка API (любой HTTP ответ = успех)
check_telegram_api() {
    local code=$(curl -s --tlsv1.3 --connect-timeout "$CURL_TIMEOUT" --max-time "$CURL_TIMEOUT" \
        -o /dev/null -w "%{http_code}" "https://api.telegram.org" 2>/dev/null)
    [[ "$code" != "000" ]]
}

# Полная проверка Telegram (с выводом)
check_telegram_full() {
    echo "Проверяем Telegram (TLS 1.3):"
    echo -n "  web.telegram.org... "
    if check_telegram_web; then
        echo "✓"
    else
        echo "✗"
        return 1
    fi
    echo -n "  api.telegram.org (API)... "
...
# Тест стратегии, возвращает "web_ok:api_ok"
test_strategy() {
    local web_ok=0 api_ok=0
    check_telegram_web && web_ok=1
    check_telegram_api && api_ok=1
    echo "$web_ok:$api_ok"
}
...
────────────────────────────────────────────────────────────────"
    ...
    for entry in "${WORKING_STRATEGIES[@]}"; do
        ...
        local web_status=$([[ $web_ok -gt 0 ]] && echo '✓' || echo '✗')
        local api_status=$([[ $api_ok -gt 0 ]] && echo '✓' || echo '✗')
        local line=$(printf "  [%-2s] %-40s Web:%s API:%s" "$num" "$name" "$web_status" "$api_status")
        echo "$line"
...
echo ""
echo -e "${BOLD}${CYAN}╔════════════════════════════════════════════════════════════╗${NC}"
echo -e "${BOLD}${CYAN}║${NC}     ${BOLD}🔧 Auto Tune для zapret-telegram${NC}                      ${BOLD}${CYAN}║${NC}"
echo -e "${BOLD}${CYAN}╚════════════════════════════════════════════════════════════╝${NC}"
echo ""
...
# Проверка без zapret
echo "Проверяем доступность Telegram без zapret..."
if check_telegram_full; then
    echo ""
    echo -e "${GREEN}✓${NC} Telegram уже доступен. Ничего делать не нужно."
    exit 0
fi

        echo ""
echo "Telegram недоступен. Тестируем стратегии..."
echo ""
...
for ((i=1; i<=MAX_STRATEGY; i++)); do
    ...
    # Стратегия успешна, если хотя бы одна проверка прошла
    if [[ $web_ok -gt 0 ]] || [[ $api_ok -gt 0 ]]; then
        if [[ $web_ok -gt 0 ]] && [[ $api_ok -gt 0 ]]; then
            echo -e "${GREEN}Web:✓ API:✓${NC}"
        elif [[ $web_ok -gt 0 ]]; then
            echo -e "${GREEN}Web:✓${NC} ${RED}API:✗${NC}"
        else
            echo -e "${RED}Web:✗${NC} ${GREEN}API:✓${NC}"
        fi
        ((SUCCESS_COUNT++))
        WORKING_STRATEGIES+=("$i:$name:$web_ok:$api_ok")
    else
        echo -e "${RED}Web:✗ API:✗${NC}"
        ((FAILED_COUNT++))
    fi
